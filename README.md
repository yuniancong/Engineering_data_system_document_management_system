...
- **加载数据**：从本地存储加载之前保存的数据
- **导出数据**：
  - **Excel格式**（已实现）：包含4个工作表（卷内目录、卷内备考表、案卷封面、案卷目录）
  - **Word模板格式**（规划中）：基于标准模板自动填充数据，导出符合标准格式的Word文档
    - 需要先上传Word模板文件到 `templates/` 文件夹
    - 支持5种模板：档案移交目录、档案封面、卷内目录、备考表、移交书
- **清空数据**：清除所有数据（需确认）
...
**功能说明**：
- 上传Word模板文件后，系统将支持基于模板自动填充数据并导出Word文档
- 导出的Word文档将符合标准格式要求，可直接打印归档
- 模板文件使用标准格式，系统会自动识别并填充对应字段

**使用方法**（待实现）：
1. 将标准Word模板文件上传到此文件夹
2. 在系统中点击"导出Word"按钮
3. 选择要导出的模板类型
4. 系统自动填充数据并下载Word文档
