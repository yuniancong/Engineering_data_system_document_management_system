...
**从Excel粘贴多列数据**：
```
1. 在Excel中选择多行数据（包含文件编号、责任者、文件题名等列）
2. 复制（Ctrl+C）
3. 在系统中点击"📋 粘贴整行"按钮
4. 系统自动识别Tab分隔的数据，创建对应数量的行并填充
```
...
```
在Excel中复制以下数据：
XM-001	建设单位	施工组织设计	2024-01-15	1-20
XM-002	监理单位	开工报告	2024-01-16	21-25	已审批

点击"📋 粘贴整行"，系统自动创建2行并填充所有数据
```
...
### 7. 批量粘贴整行时列顺序不对怎么办？
- 系统按固定顺序识别列：文件编号、责任者、文件题名、日期、页次、备注
- 在Excel中调整列顺序以匹配系统要求
- 如果只有部分列，留空即可（例如只有题名和日期，则前两列为空）
